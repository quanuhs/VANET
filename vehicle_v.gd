...
func _draw():
	...
	if connected_rsu:
		...
		draw_line(Vector2.ZERO, connected_rsu.global_position - self.global_position, _color, 2.0)
...
func stop_service(rsu):
	...
	if awaiting_response:
		...
		will_recive_time = OS.get_ticks_msec()
		wait_request_time = OS.get_ticks_msec()
